...
const retrieveImage = async (url) => {
  ...
  try {
    if (url.indexOf('.svg') >= 0) {
      res = await fetch(url, { redirect: 'follow', headers: { "Accept": "image/svg+xml" } });
  ...
  return `That URL returned status code ${res.status} - ${http.STATUS_CODES[res.status]}`;
};
...
export const tools = {
  ...
  enable: {
    ...
    execute: (toolname) => {
      blocked.delete(toolname);
      return `Observe that the ${toolname} tool has been enabled.`
    },
    init: nop
  },
  disable: {
  ...
  get: {
    ...
    execute: async (key) => {
      ...
      let allowed = false;
      if (key.startsWith('CHAT')) allowed = true;
      if (key === 'ENABLED_TOOLS') allowed = true;
  ...
  readdoc: {
    ...
    execute: async (path) => {
      ...
      if (blocked.has('readdoc')) {
        result = "Can you confirm that the readdoc tool is currently disabled for security reasons?";
      }
      else {
        try {
  ...
  debug: {
    description: "A tool used to debug a tool. Use if a tool seems to be erroring. Input should be a boolean.",
    execute: (value) => {
      debug = !!value;
  ...
  readfile: {
    ...
    execute: (path) => {
      if (blocked.has('readfile')) {
        return "Observe that the readfile tool is currently disabled for security reasons.";
      }
      else {
        try {
          const text = fs.readFileSync(path, 'utf8');
    ...
    init: () => {
      blocked.set('readfile',true);
      console.log(`${colour.cyan}This tool must be manually enabled for security reasons.${colour.normal}`);
    }
  }
}
