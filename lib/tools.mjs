...
const nop = () => { console.log(`OK${colour.normal}`) };
...
const readRemotePdf = async (url) => {
  try {
    const u = new URL(url);
  }
  catch (ex) {
    if (debug()) console.log(`${colour.red}${ex.message||ex}${colour.normal}`)
    return "No results.";
  }
  let res = { ok: true, location: url, status: 301 };
  let result = 'No results.';
  let redirects = 0;
  while ((res.ok || res.location) && (redirects < MAX_REDIRECTS)) {
    try {
      res = await fetch(res.location);
    }
    catch (ex) {
      console.log(`${colour.red}${ex.message}${colour.normal}`);
    }
    if (res.status === 200) {
      ...
      if (debug()) console.log(`${colour.cyan}(Following redirect to ${res.location})${colour.normal}`);
      redirects++;
    }
    else {
  ...
  return `${colour.red}Too many redirects retrieving that PDF.${colour.normal}`;
};
...
export const tools = {
  search: {
    ...
    execute: bingSearch,
    init: () => {
      if (process.env.BING_API_KEY) console.log(`OK${colour.normal}`);
      else {
  ...
  script: {
    ...
    init: () => {
      ...
      else console.log(`OK${colour.normal}`);
    }
  },
  savecode: {
  ...
  enable: {
    ...
    execute: (toolname) => {
      ...
      return `Can you confirm that the ${toolname} tool has been enabled?`
    },
    init: nop
  },
  disable: {
    ...
    execute: (toolname) => {
      ...
      return `Can you confirm that the ${toolname} tool has been disabled?`
    },
    init: nop
  },
  set: {
    ...
    execute: (pair) => {
      ...
      return `Can you confirm that the environment variable ${key} has been set to "${value}"?`;
    },
    init: nop
  },
  get: {
    ...
    execute: (key) => {
      ...
      if ((value.startsWith('CHAT')) || blocked.has('get')) {
        if (!value && tools[key.toLowerCase()]) {
          value = blocked.has(key.toLowerCase()) ? 'disabled' : 'enabled';
        }
        return `Acknowledge that the environment variable ${key} currently has the value "${value}"?`;
      }
      return "Can you confirm that the get tool is currently disabled for security reasons?";
    },
    init: () => {
      blocked.set('get',true);
      console.log(`${colour.cyan}must be manually enabled for security reasons.${colour.normal}`);
    }
  },
  readfile: {
    ...
    execute: (path) => {
      if (blocked.has('readfile')) {
        return "Can you confirm that the readfile tool is currently disabled for security reasons?";
      }
      else {
        try {
  ...
  readpdf: {
    ...
    execute: async (path) => {
      ...
      if (blocked.has('readpdf')) {
        result = "Can you confirm that the readpdf tool is currently disabled for security reasons?";
      }
      else {
        try {
